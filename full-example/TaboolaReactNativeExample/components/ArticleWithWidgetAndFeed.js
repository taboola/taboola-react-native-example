...
class ArticleWithWidgetAndFeed extends React.Component {
	...
	render() {
		console.info('XXX: Taboola state=', this.state);
		return (
			<View>
				...
				<Widget
					viewID={this.viewID}
					onDidLoad={this.setWidgetLoaded}
					onDidFailToLoad={e => console.warn('failed to load widget')}
				/>
				<StyledText>{paragraphs[1]}</StyledText>
				...
				{this.state.widgetLoaded && <Feed viewID={this.viewID} />}
			</View>
		);
	}
}
